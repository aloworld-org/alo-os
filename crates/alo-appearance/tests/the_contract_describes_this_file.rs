//! `docs/contracts/person-settings.md` describes `appearance.toml` as this crate
...
//!
//! **Two of these ask a Unix machine.** The file alo OS writes names a picture by
//! a whole path, and `/home/ada/harbour.jpg` is a whole path on the machine alo
//! OS is and not on a Windows host running the suite, where the crate rightly
//! refuses it. The examples a person edits with no path in them are asked
//! everywhere.
...
use alo_appearance::{Appearance, Changes, FileNotRead, appearance_words};
...
fn a_folder_of_our_own(what: &str) -> PathBuf {
    let folder = std::env::temp_dir().join(format!("alo-appearance-contract-{what}"));
    if folder.exists() {
...
fn every_example_the_contract_refuses_is_refused_in_the_words_it_names() {
    let strings = Strings::of(appearance_words().unwrap());
    let refused: Vec<Fence> = fences(&the_section())
        .into_iter()
        .filter(|fence| fence.info == "toml refused")
        .collect();
    assert!(refused.len() >= 5, "the section shows what does not read");
    for (number, fence) in refused.iter().enumerate() {
        ...
        let named = code_span_beginning(&fence.after, "appearance.kept.")
...
fn a_missing_file_is_a_person_who_changed_nothing() {
    ...
    assert_eq!(keeping::at_sign_in(&at), (Appearance::shipped(), None));
...
fn the_contract_says_a_file_that_did_not_read_is_not_written_over() {
    ...
    let named = code_span_beginning(&writing, "appearance.kept.not-replaced")
        .expect("the section names the sentence a change over a broken file is refused with");
    assert!(
        writing.contains("alo_appearance::keeping::put_back_as_shipped"),
