//! The only file that knows what a hosted provider's API looks like.
//!
...
//! since 2025 and what every provider somebody is likely to already pay for
//! offers. A provider that answers some other way is reported as one this
//! machine could not use, which is true, rather than as a bad key, which would
//! not be.
//!
//! # Four decisions carry more weight than the request does
//!
//! **Nothing here opens anything on its own.** [`Hosted::ask`] is `pub(crate)`
//! and its one caller is [`crate::Asking::to_a_provider`], which has already
//! obtained an `alo_egress::Departing` — so there is no public function in this
//! crate that reaches a provider without law 1's indicator having shown it
//! first. The guarantee is `alo-egress`' and this crate does not get a second
//! way round it.
//!
//! **A redirect is refused, never followed.** The address the rule was asked
...
//! and this is the same refusal where the stakes are the person's own work.
//!
...
/// How long this machine waits for an answer.
///
/// Longer than the ten seconds `alo_models::trying` waits, because that is
/// somebody watching a dialogue and this is a model thinking. Short enough that
/// a provider which has stopped answering is *said* to have stopped rather than
/// left looking like a machine that has hung: nothing answered in two minutes is
/// something a person needs telling about.
...
/// One question, in the shape every OpenAI-compatible provider speaks.
#[derive(Serialize)]
struct Sent<'a> {
    ...
    /// Whole answers only — this module's fourth decision.
    stream: bool,
}
...
impl<'a> Hosted<'a> {
    ...
    ///
    /// # Errors
    /// [`WentWrong`], which is `alo-answering`'s closed list rather than one of
    /// this crate's own — what went wrong where a question was put is a thing a
    /// person reads in one voice, whether the place was a provider, a paired
    /// machine or this one.
    pub(crate) fn ask(&self, question: &Question) -> Result<String, WentWrong> {
        let request = ureq::post(answers_url(&self.provider.endpoint))
            .config()
            .timeout_global(Some(WHILE_A_MODEL_THINKS))
            // Refused rather than followed — this module's second decision.
            .max_redirects(0)
            ...
            .build();
        // The key is handed the request rather than the other way round: it
        // cannot be read out of `alo-models`, and this crate never holds it as
        // text it could put anywhere else.
        ...
        match response.status().as_u16() {
            ...
            // The convention answers 404 for a model it does not offer, and
            // 405 for an address that is not the API at all. Both reach a
            // person as "what was to answer this was not there", which is what
            // both of them mean to somebody who did not type either.
            404 | 405 => return Err(WentWrong::NoModelThere),
            // A refusal of the request itself. It is not the person's to fix
            // and it is not the provider having a bad day, so it is neither
            // sentence: something answered, and not with an answer.
...
/// waiting two minutes for it — a provider that accepts the connection and then
/// says nothing — is still checked rather than assumed.
fn what_went_wrong(error: ureq::Error) -> WentWrong {
    match error {
        ureq::Error::Timeout(_) => WentWrong::TookTooLong,
        // A provider that answers a `POST` with 307 or 308 is telling this
...
/// Where a provider answers questions.
///
/// Providers document their address both ways — Mistral's ends `/v1`, a local
/// runtime's does not — and appending a second `/v1` to the first would come
/// back 404, which a person would read as *my address is wrong* when it is
/// right. `alo_models::trying` makes the same allowance for the same reason and
...
mod tests {
    use super::*;
    use crate::testing::{mistral, serving, serving_with};
    ...
    /// A provider given no key is sent no authorisation at all: a header nobody
    /// asked for is one some services refuse outright.
    #[test]
    fn a_provider_given_no_key_is_sent_no_authorisation_at_all() {
        let (url, server) = serving(AN_ANSWER, 200);
        let provider = mistral(&url);
        let answer = Hosted::provider(&provider, None).ask(&question());
        let request = server.join().unwrap();
    ...
    /// neither the key nor the provider's own words travel into what they read.
    #[test]
    fn a_key_the_provider_refuses_is_said_without_quoting_it_or_them() {
        let (url, server) = serving(r#"{"message":"Unauthorized","request_id":"abc"}"#, 401);
        let provider = mistral(&url);
        let key = Secret::typed("sk-live-0123456789").unwrap();
        let went_wrong = Hosted::provider(&provider, Some(&key))
            .ask(&question())
            .unwrap_err();
        server.join().unwrap();
    ...
    /// **The address the rule was asked about is the address that gets
    /// reached.** The redirect points at a port nothing is listening on:
    /// following it would come back as nothing answering, so the two outcomes
    /// cannot be confused.
    #[test]
    ...
    fn each_status_reaches_a_person_as_the_thing_it_actually_means() {
        for (body, status, expected) in [
            ...
            let provider = mistral(&url);
            let went_wrong = Hosted::provider(&provider, None)
                .ask(&question())
                .unwrap_err();
            server.join().unwrap();
    ...
    fn nothing_listening_is_nothing_answered() {
        let provider = mistral("http://127.0.0.1:1");
        assert_eq!(
    ...
    /// **The outcome no stub can produce in a test worth running.** A provider
    ...
    #[test]
    fn a_provider_that_goes_quiet_is_told_apart_from_one_that_was_never_there() {
        assert_eq!(
