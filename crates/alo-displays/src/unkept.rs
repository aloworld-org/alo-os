//! What a person is told when their own `displays.toml` did not read, or a
//! change to it was not written.
//!
//! [`alo_kept`] decides *when* a file is refused and hands the reason here
//! without words, because what a person reads about their screens is this
...
//!
//! **And this file's refusals say what the machine did instead**, which is the
//! part that matters here more than in most: a person whose arrangement did not
//! read is looking at screens in the wrong places, and needs to be told that is
//! why rather than left to wonder whether the machine has forgotten them.
//!
...
/// `displays.toml` is there and did not read, so nothing in it is honoured and
/// this machine lays every set of screens out as though it had never seen them.
...
impl FileNotRead {
    ...
    /// The key at the top of the file that is not part of an arrangement, when
...
/// A change to `displays.toml` was not written, so the file — and every
/// arrangement this machine remembers — is as it was.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn the_file() -> PathBuf {
        PathBuf::from("/home/ada/.config/alo/displays.toml")
    }
    ...
    fn every_reason_a_file_did_not_read_is_said_with_the_file_named() {
        ...
        for why in [
            ...
            Unread::UnknownKey {
                key: "monitors".to_owned(),
            },
            Unread::NotItsShape {
                said: "displays.no-main-screen".to_owned(),
            },
        ] {
    ...
    fn the_key_and_the_line_are_named() {
        ...
        assert_eq!(unknown.key(), Some("monitors"));
        assert!(unknown.said(&strings).text().contains("say monitors"));
