...
//!
//! `alo_displays::words`' own tests say the list is well formed. This says it
//! is **collected**: the one vocabulary a translation is checked against holds
//! every sentence about a person's screens, so it reaches them in their own
...
fn every_sentence_this_crate_can_say_is_in_the_machines_vocabulary() {
    ...
    for word in alo_displays::EVERY_WORD {
        let key = word.key();
...
/// where their screens are is not asked to learn what draws them.
#[test]
fn no_sentence_this_crate_says_names_anything_we_rent() {
    ...
    let ours: Vec<String> = overheard
        .iter()
        .filter(|one| one.key().area() == "displays")
        .map(|one| format!("{one}"))
        .collect();
