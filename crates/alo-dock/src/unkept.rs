//! What a person is told when their own `dock.toml` did not read, or a
//! change to it was not written.
//!
//! [`alo_kept`] decides *when* a file is refused and hands the reason here
//! without words, because what a person reads about their dock settings
...
/// `dock.toml` is there and did not read, so nothing in it is honoured
/// and the dock is where the release puts it.
...
impl FileNotRead {
    ...
    /// The key at the top of the file that is not a dock setting, when
...
/// A change to `dock.toml` was not written, so the file — and where the
/// dock is — is as it was.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn the_file() -> PathBuf {
        PathBuf::from("/home/ada/.config/alo/dock.toml")
    }
    ...
    fn the_key_and_the_line_are_named() {
        ...
        assert_eq!(unknown.key(), Some("size"));
        assert!(unknown.said(&strings).text().contains("say size"));
