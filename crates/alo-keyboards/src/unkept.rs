//! What a person is told when their own `keyboards.toml` did not read, or a
//! change to it was not written.
//!
//! [`alo_kept`] decides *when* a file is refused and hands the reason here
//! without words, because what a person reads about their own keyboards is this
...
//! only road to words is `said`, in the language the person reads. Which, when
//! their keyboard file is the one that did not read, may well be a language
//! they cannot currently type — so the sentence says what is true of the
//! machine right now, which is that they are typing on the keyboard alo OS
//! ships.
...
/// `keyboards.toml` is there and did not read, so nothing in it is honoured and
/// the keyboard is the one the release ships.
...
impl FileNotRead {
    ...
    /// The key at the top of the file that is not a keyboard setting, when that
    /// is what was wrong.
    #[must_use]
...
/// A change to `keyboards.toml` was not written, so the file — and which
/// keyboards a person has — is as it was.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn the_file() -> PathBuf {
        PathBuf::from("/home/ada/.config/alo/keyboards.toml")
    }
    ...
    fn the_key_and_the_line_are_named() {
        ...
        assert_eq!(unknown.key(), Some("keyboards"));
        assert!(unknown.said(&strings).text().contains("say keyboards"));
