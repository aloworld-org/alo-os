...
//!
//! `alo_leaving::words`' own tests say the list is well formed. This says it is
//! **collected**: the one vocabulary a translation is checked against holds every
//! sentence about logging out, so a person who cannot log out because an editor
//! is waiting to save reads that in their own language rather than as a key.
...
fn every_sentence_this_crate_can_say_is_in_the_machines_vocabulary() {
    ...
    for word in alo_leaving::EVERY_WORD {
        let key = word.key();
...
/// that an application has not closed is not asked to learn what a session
/// manager is.
#[test]
fn no_sentence_this_crate_says_names_anything_we_rent() {
    ...
    let ours: Vec<String> = overheard
        .iter()
        .filter(|one| one.key().area() == "leaving")
        .map(|one| format!("{one}"))
        .collect();
