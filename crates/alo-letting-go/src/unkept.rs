//! What a person is told when their own `undo.toml` did not read, or a change
//! to it was not written.
//!
//! [`alo_kept`] decides *when* a file is refused and hands the reason here
//! without words, because what a person reads about their own machine is this
//! crate's to say. Every sentence names the file, and the one about a key
//! nobody declared names the key — *your settings could not be read* without
//! either is a sentence nobody can act on.
//!
//! There is no `Display`: the only road to words is `said`, in the language the
//! person reads.
//!
//! # And what the machine does meanwhile
//!
//! It keeps what alo OS ships, for the whole of ADR 0045's reason: a window
//! that reaches nothing is undo switched off by arithmetic, and a file that
//! will not read must not become one by accident. A person whose file is wrong
//! has the shipped window and a sentence telling them so — never a machine that
//! quietly stopped keeping anything, and never one that quietly stopped letting
//! go.
...
/// `undo.toml` is there and did not read, so nothing in it is honoured and this
/// machine keeps what an agent changed for as long as the release ships.
...
/// A change to `undo.toml` was not written, so the file — and how far back this
/// machine keeps what an agent changed — is as it was.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl FileNotRead {
    ...
    /// The key at the top of the file that is not this setting, when that is
    /// what was wrong.
    #[must_use]
...
mod tests {
    ...
    fn the_file() -> PathBuf {
        PathBuf::from("/home/ada/.config/alo/undo.toml")
    }
    ...
    fn the_key_and_the_line_are_named() {
        ...
        assert_eq!(unknown.key(), Some("never-expire"));
        assert!(unknown.said(&strings).text().contains("never-expire"));
    ...
    /// apart from a fault in alo OS and from a file that was kept.
    #[test]
