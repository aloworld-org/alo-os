//! `docs/contracts/person-settings.md` describes `undo.toml` as this crate
...
use alo_letting_go::keeping::{self, FORMAT, THE_FILE};
use alo_letting_go::{Changes, FileNotRead, Settings, letting_go_words};
...
fn a_folder_of_our_own(what: &str) -> PathBuf {
    let folder = std::env::temp_dir().join(format!("alo-letting-go-contract-{what}"));
    if folder.exists() {
...
/// The window changed, so every key is written — the value the section's
/// first example is of.
...
fn the_folders_table_names_this_file_and_its_count_is_the_rows_it_has() {
    ...
    assert!(
        row.contains("`alo_letting_go::keeping`"),
...
/// and this machine then keeps what an agent changed for as long as the
/// release ships, with nothing in the file honoured.
#[test]
fn every_example_the_contract_refuses_is_refused_in_the_words_it_names() {
    let strings = Strings::of(letting_go_words().unwrap());
    ...
    for (number, fence) in refused.iter().enumerate() {
        ...
        let named = code_span_beginning(&fence.after, "letting-go.kept.")
...
fn the_contract_says_a_file_that_did_not_read_is_not_written_over() {
    ...
    let named = code_span_beginning(&writing, "letting-go.kept.not-replaced")
        .expect("the section names the sentence a change over a broken file is refused with");
    assert!(
        writing.contains("alo_letting_go::keeping::put_back_as_shipped"),
