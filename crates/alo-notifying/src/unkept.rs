//! What a person is told when their own `notifying.toml` did not read, or a
//! change to it was not written.
//!
//! [`alo_kept`] decides *when* a file is refused and hands the reason here
//! without words, because what a person reads about their notification settings
...
/// `notifying.toml` is there and did not read, so nothing in it is honoured
/// and notifications behave as the release ships them.
...
impl FileNotRead {
    ...
    /// The key at the top of the file that is not a notification setting, when
...
/// A change to `notifying.toml` was not written, so the file — and how
/// notifications behave — is as it was.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn the_file() -> PathBuf {
        PathBuf::from("/home/anna/.config/alo/notifying.toml")
    }
    ...
    fn the_key_and_the_line_are_named() {
        ...
        assert_eq!(unknown.key(), Some("loud"));
        assert!(unknown.said(&strings).text().contains("say loud"));
    ...
    fn every_reason_a_change_was_not_written_is_said() {
        ...
        for (why, word) in [
            ...
            let said = refused.said(&strings);
            assert!(said.unfilled().is_empty(), "{why:?}: {said}");
            assert!(said.text().contains("notification"), "{said}");
            assert_eq!(
