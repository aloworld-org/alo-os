...
//!
//! `alo_notifying::words`' own tests say the list is well formed. This says it
//! is **collected**: the one vocabulary a translation is checked against holds
//! every sentence a notification can cause, so it reaches a person in their own
//! language rather than as a key.
...
fn every_sentence_this_crate_can_say_is_in_the_machines_vocabulary() {
    ...
    for word in alo_notifying::EVERY_WORD {
        let key = word.key();
...
/// their notifications are waiting is not asked to learn what a portal is.
#[test]
fn no_sentence_this_crate_says_names_anything_we_rent() {
    ...
    let ours: Vec<String> = overheard
        .iter()
        .filter(|one| one.key().area() == "notifying")
        .map(|one| format!("{one}"))
        .collect();
