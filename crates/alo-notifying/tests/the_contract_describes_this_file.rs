//! `docs/contracts/person-settings.md` describes `notifying.toml` as this
...
//!
//! **The section's claim that no notification is in this file is held here as
//! a refusal**, not as a paragraph: the example naming `missed` is refused with
//! the key named, the way a hand-edited file carrying somebody's messages would
//! be.
//!
...
use alo_notifying::keeping::{self, FORMAT, THE_FILE};
use alo_notifying::{Changes, FileNotRead, QuietHours, Settings, notifying_words};
...
fn a_folder_of_our_own(what: &str) -> PathBuf {
    let folder = std::env::temp_dir().join(format!("alo-notifying-contract-{what}"));
    if folder.exists() {
...
/// Both settings changed, so every key is written — the value the section's
/// first example is of.
fn everything_changed() -> Changes {
    let mut changes = Changes::untouched();
    changes.set_do_not_disturb(true);
    changes.set_quiet_hours(
        QuietHours::these_two_times(
            TimeOfDay::checked(22, 30).unwrap(),
            TimeOfDay::checked(7, 0).unwrap(),
        )
        .unwrap(),
    );
    changes
...
fn the_folders_table_names_this_file_and_its_count_is_the_rows_it_has() {
    ...
    assert!(
        row.contains("`alo_notifying::keeping`"),
...
/// first of them as exactly the changes it was written from.
#[test]
...
/// and notifications then behave as the release ships them, with nothing in the
/// file honoured.
#[test]
fn every_example_the_contract_refuses_is_refused_in_the_words_it_names() {
    let strings = Strings::of(notifying_words().unwrap());
    ...
    for (number, fence) in refused.iter().enumerate() {
        ...
        let named = code_span_beginning(&fence.after, "notifying.kept.")
...
fn the_contract_says_a_file_that_did_not_read_is_not_written_over() {
    ...
    let named = code_span_beginning(&writing, "notifying.kept.not-replaced")
        .expect("the section names the sentence a change over a broken file is refused with");
    assert!(
        writing.contains("alo_notifying::keeping::put_back_as_shipped"),
