...
pub(crate) const THE_MP4_FAMILY: [&[u8; 4]; 28] = [
    ...
    // `hevc` is here for the same reason `avc1` is: a file whose major brand
    // names the codec rather than the container is still a film, and a machine
    // that called one *unrecognised* would be refusing to play something it can
    // plainly read.
    b"mp41", b"mp42", b"mp71", b"avc1", b"hevc", b"dash", b"cmfc", b"mmp4",
