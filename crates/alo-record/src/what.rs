...
//! approved, generated from the validated arguments and from nothing the model
...
impl What {
    ...
    pub fn of(call: &Call) -> Self {
        Self {
            ...
            effect: call.effect(),
            sentence: Line::of(call.sentence()),
            arguments: call
                .values()
                .iter()
...
mod tests {
    ...
    fn what_ran_is_kept_in_the_words_a_person_read() {
        let what = What::of(&archiving_march());
    ...
    fn a_read_is_recorded_like_anything_else() {
        let what = What::of(&listing_invoices());
