...
/// Use stage one's mapped root, child, popup and cursor without dispatching.
pub fn run(server: &Server, renderer: &mut GlesRenderer) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for scheme in [Scheme::Light, Scheme::Dark] {
        ...
        for cursor_origin in [(10, 14), (10, 50)] {
            ...
            let removed = render_control_scanout(
                renderer,
                (320, 180).into(),
                &roots,
                &popups,
    ...
    assert_eq!(frames, 8);
    println!(
        "Native scene: eight custom-cursor and four arrow complete 57,600-pixel frames, removal and refusal recovery passed"
    );
    Ok(())
}
