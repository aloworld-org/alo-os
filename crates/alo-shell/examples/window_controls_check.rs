...
fn run() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for restoring in [false, true] {
            for mask in 0_u8..8 {
                ...
                for origin in [(3, 4), (-5, -7), (100, 35), (120, 48)] {
                    let layout = WindowControlLayout::new((120, 48), origin, enabled, restoring)?;
                    let mut frame =
                    ...
                    for (index, pixel) in rows.iter().enumerate() {
                        ...
                        let expected = window_controls_pixels::expected(
                            (x, y),
                            origin,
                            scheme,
                            enabled,
    ...
    println!(
        "Native window controls: 128 full 5,760-pixel GLES frames passed (light/dark, all availability combinations, maximize/restore, four clipping positions); view-only evidence"
    );
    Ok(())
}
