...
impl crate::presentation::NativeTarget for Target {}
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    for pause in [false, true] {
        ...
        // before the next statement ran: libinput holds its own duplicate and
        // lets go during the context's shutdown. `crate::descriptor_testing`
        // says the rest, and is the one place this waiting lives — a second
        // test had the same fault and the same fix.
        assert_eq!(
            crate::descriptor_testing::reached_end_of_file(peer)?,
            0,
            "the session's descriptor never reached end-of-file"
        );
    }
    Ok(())
}
