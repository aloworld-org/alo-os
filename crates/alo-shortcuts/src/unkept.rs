//! What a person is told when their own `shortcuts.toml` did not read, or a
//! change to it was not written.
//!
//! [`alo_kept`] decides *when* a file is refused and hands the reason here
//! without words, because what a person reads about their shortcut settings
...
/// `shortcuts.toml` is there and did not read, so nothing in it is honoured
/// and every shortcut is the one the release ships.
...
impl FileNotRead {
    ...
    /// The key at the top of the file that is not a shortcut setting, when
...
/// A change to `shortcuts.toml` was not written, so the file — and what
/// every key does — is as it was.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn the_file() -> PathBuf {
        PathBuf::from("/home/ada/.config/alo/shortcuts.toml")
    }
    ...
    fn the_key_and_the_line_are_named() {
        ...
        assert_eq!(unknown.key(), Some("bindings"));
        assert!(unknown.said(&strings).text().contains("say bindings"));
