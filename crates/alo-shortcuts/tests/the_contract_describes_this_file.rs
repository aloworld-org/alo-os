//! `docs/contracts/person-settings.md` describes `shortcuts.toml` as this crate
...
use alo_shortcuts::{
    Action, Changes, Chord, FileNotRead, Key, Modifier, Modifiers, Shortcuts, shortcut_words,
};
...
fn a_folder_of_our_own(what: &str) -> PathBuf {
    let folder = std::env::temp_dir().join(format!("alo-shortcuts-contract-{what}"));
    if folder.exists() {
...
/// A person who moved the agent, cleared the launcher and moved the next
/// window — the value the section's first example is of.
...
fn every_example_the_contract_refuses_is_refused_in_the_words_it_names() {
    let strings = Strings::of(shortcut_words().unwrap());
    ...
    for (number, fence) in refused.iter().enumerate() {
        ...
        let named = code_span_beginning(&fence.after, "shortcuts.kept.")
...
fn the_contract_says_a_file_that_did_not_read_is_not_written_over() {
    ...
    let named = code_span_beginning(&writing, "shortcuts.kept.not-replaced")
        .expect("the section names the sentence a change over a broken file is refused with");
    assert!(
        writing.contains("alo_shortcuts::keeping::put_back_as_shipped"),
