//! `docs/contracts/person-settings.md` describes `sleeping.toml` as this crate
...
use alo_sleeping::keeping::{self, FORMAT, THE_FILE};
use alo_sleeping::{Changes, FileNotRead, Lid, Settings, sleeping_words};
...
fn a_folder_of_our_own(what: &str) -> PathBuf {
    let folder = std::env::temp_dir().join(format!("alo-sleeping-contract-{what}"));
    if folder.exists() {
...
fn the_folders_table_names_this_file_and_its_count_is_the_rows_it_has() {
    ...
    assert!(
        row.contains("`alo_sleeping::keeping`"),
...
/// and this machine then sleeps the way the release ships it, with nothing in
/// the file honoured.
#[test]
fn every_example_the_contract_refuses_is_refused_in_the_words_it_names() {
    let strings = Strings::of(sleeping_words().unwrap());
    ...
    for (number, fence) in refused.iter().enumerate() {
        ...
        let named = code_span_beginning(&fence.after, "sleeping.kept.")
...
fn the_contract_says_a_file_that_did_not_read_is_not_written_over() {
    ...
    let named = code_span_beginning(&writing, "sleeping.kept.not-replaced")
        .expect("the section names the sentence a change over a broken file is refused with");
    assert!(
        writing.contains("alo_sleeping::keeping::put_back_as_shipped"),
