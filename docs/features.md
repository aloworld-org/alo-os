...
- [v0.5] **Selling inference means we profit when questions leave the machine**, which runs against this product's own promise. It is written into ADR 0014 rather than left unsaid, and the defence is structural: our service is configured exactly as a third party is, so there is nowhere for a special case to live
- [v0.5] **Cancelling is not an expiry.** Signing in to alo OS does not sign anybody in to a paid model, and stopping paying leaves an operating system that works rather than a trial that ended
